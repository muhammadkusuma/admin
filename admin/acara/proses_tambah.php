...
if (isset($_POST['simpan'])) {
    ...
    if ($filename != "") {
        ...
        if (in_array(strtolower($ext), $ekstensi_diperbolehkan)) {
            if ($ukuran < 2097152) { // Max 2MB
                ...
                echo "<script>alert('Ukuran poster terlalu besar! Maksimal 2MB.'); window.location='tambah.php';</script>";
                exit;
            }
        } else {
    ...
    // Insert ke Database
    $query = "INSERT INTO acara (judul, kategori, tanggal, jam, lokasi, deskripsi, poster, status) 
              VALUES ('$judul', '$kategori', '$tanggal', '$jam', '$lokasi', '$deskripsi', '$nama_poster', '$status')";
